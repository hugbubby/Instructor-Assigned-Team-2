...
public class KioskController {
	...
	//PIV
	private Inventory IKiosk = new Inventory();						//Inventory Object to hold kiosk inventory
	private ArrayList<Product> alInventory = IKiosk.getInventory(); //Arraylist of Products in Inventory
	...
	public void buttonKioskAdminPageClicked(Event event){
		...
		Parent kiosk = null;
		try {
			kiosk = FXMLLoader.load(getClass().getResource("Admin.fxml"));
		} catch (IOException e) {
			e.printStackTrace();
		}
	...
	public void buttonKioskSearchClicked(Event event){
		if (textFieldKioskSearch.getText().toString().equals("")) {
			bSearch = false;
			//search is null set to first page
			buttonPreviousPage.visibleProperty().setValue(false);
			buttonNextPage.visibleProperty().setValue(true);
			...
			bSearch = true;
			iPosition = 0;//page 0
			alResults = IKiosk.searchByHash(textFieldKioskSearch.getText().toString().toLowerCase());
	 ...
	 */
	public void loadProducts(ArrayList<Product> inventory){
		boolean end = false;
	 ...
	 * 	
	 */
	public String printProduct(ArrayList<Product> inventory) {
